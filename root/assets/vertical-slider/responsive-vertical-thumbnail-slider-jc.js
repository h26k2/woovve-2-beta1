...
;(function($){
	...
	$.fn.bxSlider_vertical = function(options){

		if(this.length == 0) return this;

		// support mutltiple elements
		if(this.length > 1){
			this.each(function(){$(this).bxSlider_vertical(options)});
			return this;
		}
		...
		var getViewportHeight = function(){
			...
			// if not "vertical" mode, calculate the max height of the children
			 ...
			 if(slider.settings.mode == 'vertical'){
                            ...
                            var attrDataHeight = $(el).attr('data-height');

                            if (typeof attrDataHeight !== typeof undefined && attrDataHeight !== false) {
                                heightOfslider=$(el).attr('data-height');
                                heightOfslider.replace("px", "");
                                heightOfslider.replace("PX", "");
                                height=parseFloat(heightOfslider);
		...
		var getSlideWidth = function(){
			...
			if(slider.settings.slideWidth == 0 ||
				...
				slider.settings.mode == 'vertical'){
                                /*if($(window).width()-28<=wrapWidth){


                                    wrapWidth=$(window).width()-28;
                                    $(el).parent().width($(window).width()-28);

                                   $(el).parent().parent().parent().css('float','left');

                                }else{

                                    $(el).parent().width('100%');
                                    $(el).parent().parent().parent().css('float','');
			...
			slider.children.each(function(index){
				...
				if (title != undefined && ('' + title).length) {
                                    if (slider.settings.mode == 'vertical'){
                                        ...
                                        $(this).append('<div class="bx-caption-vertical"><span>' + title + '</span></div>');
	...
	parentVerticalPadding = slider.viewport.innerHeight() - slider.viewport.height();
      ...
      // In case slider parent have vertical padding recalculate position
